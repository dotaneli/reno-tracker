...
export interface Financials {
  ...
  remainingPayments: number; // milestoned - paid
  budgetRemaining: number;   // budget - totalCost
  paidPct: number;         // paid / milestoned
...
export function useFinancials(projectId: string | undefined): Financials & { loading: boolean } {
  ...
  return useMemo(() => {
    ...
    const remainingPayments = totalMilestoned - totalPaid;
    const budgetRemaining = totalBudget - totalCost;
    const paidPct = totalMilestoned > 0 ? Math.round((totalPaid / totalMilestoned) * 100) : 0;
    ...
    return {
      ...
      remainingPayments, budgetRemaining, paidPct, costPct,
