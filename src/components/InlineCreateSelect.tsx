    ...
    return (
      <div className={`space-y-1 ${className}`}>
        <div className="flex gap-1">
          ...
          <button type="button" onClick={(e) => { e.preventDefault(); setCreating(false); setNewName(""); setCreateError(""); }}
            className="shrink-0 rounded-lg bg-[var(--border-subtle)] p-2 text-[var(--fg-secondary)]"><X size={14} /></button>
        </div>
        {createError && <p className="text-[10px] text-[var(--alert)]">{createError}</p>}
      </div>
    );
  }
