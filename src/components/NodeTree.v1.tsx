...
export function NodeTree({ nodes, depth = 0, projectId, vendors, categories, floors, allNodes, editNodeId, onMutate, onEdit, onEditDone, onEditCancel, tr, parentVendor, parentCost, allProjectMilestones }: NodeTreeProps) {
  ...
  const handleDragEnd = useCallback(async (event: DragEndEvent) => {
    ...
    if (targetId === "root-drop" || targetId === "root-drop-area" || targetId === "root-drop-bottom") {
      // Avoid no-op when dragging an already-root node to root
  ...
  if (depth === 0) {
    return (
      <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        <RootDropArea active={!!activeId} label={t("task.moveToRoot")}>
...
function RootDropArea({ active, label, children }: { active: boolean; label: string; children: React.ReactNode }) {
